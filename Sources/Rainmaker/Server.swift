...
public final class Server: Serving {
    ...
    ///
    /// Convert an item's absolute href into a path relative to the WebDAV root.
    ///
    private func relativePath(for item: Item) -> String {
        let basePath = webDAVAddress.path()
        let itemPath = item.href.path()

        guard itemPath.hasPrefix(basePath) else {
            return itemPath
        }

        let remainder = itemPath.dropFirst(basePath.count)

        guard !remainder.isEmpty else {
            return "/"
        }

        return remainder.hasPrefix("/") ? String(remainder) : "/\(remainder)"
    }
            ...
            Task {
                do {
                    ...
                    for item in items {
                        ...
                        let nestedPath = relativePath(for: item)
                        logger.debug("Entering subdirectory: \(nestedPath)")
                        let nestedItems: AsyncThrowingStream<Item, Error> = try await enumerate(at: nestedPath, recursively: true)
